...
* --------------------
* ver 3.6 : 05 Apr 2016
* - Added the [], -> as two character tokens
* ver 3.5 : 10 Feb 2016
...
namespace Scanner
{
  ...
  public:
	...
	ConsumeState* nextStatePunc(int chNext);
...
void ConsumeState::clearEOF(int chNext) {
	if (chNext == EOF)
		_pContext->_pIn->clear();
}
/* ---make nextstate function samll--- */
ConsumeState* ConsumeState::nextStatePunc(int chNext) {
...
ConsumeState* ConsumeState::nextState()
{
  ...
  clearEOF(chNext);
  ConsumeState*state = nextStatePunc(chNext);
  if (state != nullptr)
	  return state;
...
int main()
{
  ...
  catch (std::logic_error& ex) { std::cout << "\n  " << ex.what(); }
  std::cout << "\n\n";
  return 0;
}
#endif
